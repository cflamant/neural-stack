...
if __name__ == "__main__":
    ...
    filenames = ['neural-stack/models/lstm_256h_1l_50b_Adam_1e-3',
                 'neural-stack/models/lstm_256h_1l_50b_Adam_1e-4',
                 'neural-stack/models/lstm_256h_1l_50b_SGD_1e-3',
                 'neural-stack/models/lstm_256h_1l_50b_SGD_1e-4'
                ]
    ...
    axval.legend()
    axval.set_title("LSTM, 256h, 1l, 50b, Validation Accuracy")
    axval.set_ylabel("fine accuracy")
    axval.set_xlabel("batch number")
    figval.savefig('neural-stack/plots/lstmbig_val.pdf', bbox_inches='tight')

    figtest, axtest = plt.subplots(1,1)
    for i, fname in enumerate(filenames):
        axtest.plot(accbatch[i], test_accs[i], label=labels[i])
    axtest.legend()
    axtest.set_title("LSTM, 256h, 1l, 50b, Test Accuracy")
    axtest.set_ylabel("fine accuracy")
    axtest.set_xlabel("batch number")
    figtest.savefig('neural-stack/plots/lstmbig_test.pdf', bbox_inches='tight')
