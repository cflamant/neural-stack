...
if __name__ == "__main__":
    ...
    savefile = 'neural-stack/models/stack_16h_16e_1l_50b_Adam_1e-3_accfreq1000'
    overwrite = True

    # hyperparameters
    hidden_dim = 16    # Hidden size
    embedding_dim = 16  # Embedding size for neural stack
    ...
    lstmstack = reverser.LSTMandStack_Reverser(use_cuda=True, hidden_dim=hidden_dim, in_dim=in_dim, out_dim=out_dim,embedding_dim=embedding_dim, max_len=max_len, num_layers=num_layers)
    rev = reverser.Reverser(lstmstack, optimizer=optimizer, lr=lr)
